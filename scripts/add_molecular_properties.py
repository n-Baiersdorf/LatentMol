...
Beispielverwendung:
    python add_molecular_properties.py --input path/to/sequences.json --mol-dir path/to/mol_files --output path/to/output.json
    python add_molecular_properties.py --input path/to/directory --mol-dir path/to/mol_files --recursive
"""
...
try:
    from PreprocessingPipeline.MolecularProperties import add_properties_to_mol_sequences
...
def process_file(file_path, mol_dir, output_dir=None, num_processes=None):
    """
    ...
    try:
        ...
        success = add_properties_to_mol_sequences(
            file_path,  # JSON-Datei mit Sequenzen
            mol_dir,    # Verzeichnis mit MOL-Dateien
...
def process_directory(input_dir, mol_dir, output_dir=None, recursive=False, num_processes=None):
    """
    Verarbeitet alle JSON-Dateien in einem Verzeichnis.
    
    Args:
        input_dir: Eingabeverzeichnis mit JSON-Dateien
        mol_dir: Verzeichnis mit MOL-Dateien
        output_dir: Ausgabeverzeichnis (optional)
    ...
    for file_path in tqdm(json_files, desc="Dateien", unit="file"):
        ...
        if output_dir:
            ...
            if process_file(file_path, mol_dir, file_output_dir, num_processes):
                successful += 1
        else:
            # Überschreibe die Originaldateien
            if process_file(file_path, mol_dir, None, num_processes):
                successful += 1
...
def main():
    ...
    parser.add_argument("--input", required=True, help="Eingabedatei oder -verzeichnis mit JSON-Sequenzdaten")
    parser.add_argument("--mol-dir", required=True, help="Verzeichnis mit den originalen MOL-Dateien")
    ...
    # Prüfe ob das MOL-Verzeichnis existiert
    if not os.path.exists(args.mol_dir) or not os.path.isdir(args.mol_dir):
        logging.error(f"MOL-Verzeichnis existiert nicht oder ist kein Verzeichnis: {args.mol_dir}")
        return 1
    ...
    if os.path.isfile(args.input):
        ...
        success = process_file(args.input, args.mol_dir, output_dir, num_processes)
        
        return 0 if success else 1
    else:
        # Verarbeite Verzeichnis
        successful = process_directory(args.input, args.mol_dir, args.output, args.recursive, num_processes)
